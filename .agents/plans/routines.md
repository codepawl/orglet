...
UI: existing monochrome drawers, native time/select/checkbox controls; routine list shows next run and one explicit catch-up action. Configure from selected brief/sources, manage from sidebar. Preserve existing type/color tokens. Verify timezone/DST, restart, atomic claims, consent, source changes, concurrency, shift handoff, backup and native UI.
