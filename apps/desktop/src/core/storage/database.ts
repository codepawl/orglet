...
export class Store {
  ...
  private recentAppChanges(): Workspace['recentAppChanges'] {
    const rows = this.db.prepare('SELECT data FROM app_proposals ORDER BY rowid DESC LIMIT 200').all();
    const applied = rows.map(row => AppProposal.parse(JSON.parse(String(row.data)))).filter(proposal => proposal.status === 'applied' && proposal.appliedAt);
    applied.sort((first, second) => second.appliedAt!.localeCompare(first.appliedAt!));
    return applied.slice(0, 30).map(proposal => ({
      id: proposal.id, taskId: proposal.taskId, kind: proposal.kind, action: proposal.action, title: proposal.title,
      automatic: proposal.automatic === true, appliedAt: proposal.appliedAt!, workerName: this.proposingWorkerName(proposal.runId),
    }));
  }
  private proposingWorkerName(runId: string) {
