 ...
 * order the crew will take it, and the combining step waits for the members.
 */
export function crewWaits(pending: readonly PendingAssignment[], delivered: ReadonlySet<string>, runs: ReadonlyMap<string, Run>, synthesis: Run, since: number): TeamWait[] {
  const waits: TeamWait[] = [];
  let ahead = 0;
  for (const assignment of pending) {
    const run = runs.get(assignment.workerId);
    if (!run) continue;
    const missing = (assignment.dependsOn ?? []).filter(workerId => !delivered.has(workerId));
    if (missing.length) {
