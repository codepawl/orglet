...
export const orglet = new Proxy({} as Bridge, {
  get(_target, key: keyof Bridge) {
    ...
    if (typeof value !== 'function' || key === 'onChange' || key === 'onProgress' || key === 'onUpdate' || key === 'onNavigate' || key === 'onOpenChat' || key === 'onOpenTask' || key === 'onIncoming' || key === 'onBrowserLive') return value;
