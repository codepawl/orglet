...
export function TaskDialog({ open, task, workspace, usedMicros, onClose }: { open: boolean; task?: Task; workspace: Workspace; usedMicros: number; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    if (mode === 'workers' && !chosen.length) { setTab('general'); setError(t('Chọn ít nhất một nhân viên.')); return; }
    if (!Number.isFinite(budgetMicros) || budgetMicros < 1000 || budgetMicros > 100_000_000) { setTab('limits'); setError(t('Nhập từ {0} đến {1}.', [formatMoney(1000), formatMoney(100_000_000)])); return; }
  ...
  return <TabbedFormDialog open={open} onClose={onClose} title={t('Thiết lập công việc')} tabs={tabs} tab={tab} onTab={next => { setTab(next); setError(''); }} panelId="task-panel" onSubmit={() => void submit()} submitLabel={t('Lưu công việc')} busy={busy || running}>
    {tab === 'general' && <>
      ...
      <Select label={<FieldLabel icon={UserRound} required>{t('Giao cho')}</FieldLabel>} value={mode} onChange={setMode} options={[
      ...
      {mode === 'workers' && <fieldset className="assignee-list"><legend className="visually-hidden">{t('Chọn nhân viên')}</legend>
        {workspace.workers.map(worker => <Checkbox key={worker.id} aria-label={worker.name} checked={chosen.includes(worker.id)} onChange={event => setChosen(current => event.target.checked ? [...current, worker.id] : current.filter(id => id !== worker.id))}>
    ...
    {tab === 'limits' && <>
      <label><FieldLabel icon={Wallet} required>{t('Giới hạn chi phí của công việc')}</FieldLabel><MoneyInput type="number" min="0" step="any" value={budget} onChange={setBudget} /></label>
