...
/**
 * Vertical-dots menu for a sidebar row. The popover is fixed to the viewport so the scrolling sidebar
 * cannot clip it; it closes on selection, outside click, Escape or focus leaving the menu.
 */
export function RowMenu({ label, items, icon: Icon = EllipsisVertical, className = 'row-action', align = 'end' }: { label: string; items: RowMenuItem[]; icon?: LucideIcon; className?: string; align?: 'start' | 'end' }) {
  ...
  useEffect(() => { if (open) root.current?.querySelector<HTMLButtonElement>('[role=menuitem]')?.focus(); }, [open, asking]);
  useEffect(() => {
    if (!open) return;
  ...
  return <div ref={root} className="row-menu" onBlur={event => { if (open && !root.current?.contains(event.relatedTarget as Node | null)) close(); }} onKeyDown={event => {
    if (!open) return;
    ...
    {open && <div className="row-menu-popover" role="menu" aria-label={label} style={position}>
      {asking ? <>
