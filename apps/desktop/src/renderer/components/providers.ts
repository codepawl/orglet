...
export function readiness(connections: Connections, harnesses: HarnessInfo[]): Readiness {
  const harnessReady = (id: 'claude-code' | 'codex' | 'cursor') => harnesses.some(item => item.id === id && item.auth !== 'logged_out');
  return { ...connections, 'claude-code': harnessReady('claude-code'), codex: harnessReady('codex'), cursor: harnessReady('cursor') };
}
