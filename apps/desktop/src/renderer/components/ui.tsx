...
export function MoneyInput({ value, onChange, invalid, ...props }: Omit<ComponentProps<'input'>, 'value' | 'onChange'> & { value: string; onChange: (value: string) => void; invalid?: boolean }) {
  return <span className={cn('money-input', invalid && 'invalid')}><span aria-hidden="true">{moneySymbol()}</span><input {...props} inputMode="decimal" value={value} aria-invalid={invalid || undefined} onChange={event => onChange(event.target.value)} /><span className="money-currency" aria-hidden="true">{displayCurrency().code}</span></span>;
}
