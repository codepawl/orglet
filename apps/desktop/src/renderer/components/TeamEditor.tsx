...
export function TeamDialog({ open, team, workspace, onClose }: { open: boolean; team?: Team; workspace: Workspace; onClose: () => void }) {
  ...
  const fail = (at: Tab, message: string, focus?: string) => {
    setTab(at); setError(message);
    if (focus) setTimeout(() => (document.querySelector(`#team-panel ${focus}`) as HTMLElement | null)?.focus(), 0);
  };
  ...
  const submit = () => {
    if (!name.trim()) return fail('general', t('Nhập tên nhóm.'), 'input');
    if (!members.length || members.length > 4) return fail('general', t('Chọn từ 1 đến 4 thành viên.'));
    if (!instructions.trim()) return fail('instructions', t('Hướng dẫn của nhóm không được để trống.'), 'textarea');
    if (requiredChecks.length && !ReviewPolicy.safeParse({ requiredChecks }).success) return fail('checklist', t('Mỗi mục kiểm tra cần tên riêng, tối đa 200 ký tự; không để trống hoặc trùng tên.'), 'input');
    const monthlyBudgetMicros = toMicros(limit), taskBudgetMicros = toMicros(taskBudget);
    if (![monthlyBudgetMicros, taskBudgetMicros].every(value => Number.isFinite(value) && value >= 0)) return fail('limits', t('Giới hạn chi phí phải là số không âm.'));
    if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 4) return fail('limits', t('Số công việc chạy đồng thời từ 1 đến 4.'));
    if (shift && !TimeZone.safeParse(shiftZone).success) return fail('limits', t('Timezone của ca không hợp lệ. Dùng tên như Asia/Ho_Chi_Minh hoặc UTC.'), 'input[aria-describedby="shift-zone-error"]');
    if (shift && (!shiftDays.length || shiftStart === shiftEnd)) return fail('limits', t('Chọn ít nhất một ngày làm việc và giờ bắt đầu khác giờ kết thúc.'));
    void run(async () => {
  ...
  return <TabbedFormDialog open={open} onClose={onClose} title={team ? t('Thiết lập nhóm') : t('Nhóm mới')} tabs={tabs} tab={tab} onTab={next => { setTab(next); setError(''); }} panelId="team-panel" onSubmit={submit} submitLabel={t('Lưu nhóm')} busy={busy} actions={actions} description={tab === 'checklist' ? t('Báo cáo tổng hợp phải đề cập các mục này. Mục thiếu bằng chứng được giữ là chưa đánh giá. Thay đổi chỉ áp dụng cho lần chạy mới.') : undefined}>
    {tab === 'general' && <>
      ...
      <label><FieldLabel icon={Users} required>{t('Tên nhóm')}</FieldLabel><input value={name} onChange={e => setName(e.target.value)} maxLength={80} /></label>
      <fieldset><legend><FieldLabel icon={Users} required>{t('Thành viên (1–4)')}</FieldLabel></legend>{workspace.workers.map(worker => <Checkbox key={worker.id} aria-label={worker.name} checked={members.includes(worker.id)} onChange={e => setMembers(current => e.target.checked ? [...current, worker.id] : current.filter(id => id !== worker.id))}><span className="inline-mark"><Avatar name={worker.name} seed={worker.id} emoji={worker.avatar?.emoji} mascot={worker.avatar?.mascot} defaultMascot hint={worker.description} color={worker.avatar?.color} size="xs" badge={worker.provider === 'demo' ? undefined : <ProviderMark provider={worker.provider} size="small" decorative />} />{worker.name}</span></Checkbox>)}{!workspace.workers.length && <p className="muted">{t('Chưa có nhân viên. Tạo nhân viên trước hoặc bắt đầu từ template.')}</p>}</fieldset>
    ...
    {tab === 'instructions' && <textarea aria-label={t('Hướng dẫn của nhóm')} aria-required="true" rows={12} value={instructions} onChange={e => setInstructions(e.target.value)} maxLength={16000} />}
    {tab === 'checklist' && <>
      {requiredChecks.map((check, index) => <fieldset key={index}><legend>{t('Mục {0}', [index + 1])}</legend>
        <div className="form">
          <label><FieldLabel icon={Type} required>{t('Tên mục')}</FieldLabel><input aria-label={t('Tên mục {0}', [index + 1])} value={check.name} maxLength={200} onChange={event => setRequiredChecks(current => current.map((item, position) => position === index ? { ...item, name: event.target.value } : item))} /></label>
    ...
    {tab === 'limits' && <>
      <label><FieldLabel icon={Wallet} required>{t('Giới hạn nhóm / tháng')}</FieldLabel><MoneyInput type="number" min="0" step="any" value={limit} onChange={setLimit} /></label>
      <label><FieldLabel icon={Wallet} required>{t('Giới hạn mỗi task')}</FieldLabel><MoneyInput type="number" min="0" step="any" value={taskBudget} onChange={setTaskBudget} /></label>
      <label><FieldLabel icon={Layers} required>{t('Số công việc chạy đồng thời')}</FieldLabel><input type="number" min="1" max="4" step="1" value={concurrency} onChange={event => setConcurrency(Number(event.target.value))} /></label>
      <Checkbox checked={shift} onChange={event => setShift(event.target.checked)}>{t('Giới hạn khung giờ làm việc')}</Checkbox>
      {shift && <>
        <label><FieldLabel icon={Globe} required>{t('Timezone của ca')}</FieldLabel><input value={shiftZone} onChange={event => { setShiftZone(event.target.value); if (zoneError) setError(''); }} maxLength={100} aria-invalid={zoneError} aria-describedby="shift-zone-error" /></label>
        {zoneError && <p id="shift-zone-error" role="alert" className="error">{error}</p>}
        <label><FieldLabel icon={Clock} required>{t('Bắt đầu ca')}</FieldLabel><input type="time" value={shiftStart} onChange={event => setShiftStart(event.target.value)} /></label>
        <label><FieldLabel icon={Clock} required>{t('Kết thúc ca')}</FieldLabel><input type="time" value={shiftEnd} onChange={event => setShiftEnd(event.target.value)} /></label>
        <fieldset><legend><FieldLabel icon={CalendarDays} required>{t('Ngày bắt đầu ca')}</FieldLabel></legend>{[t('Chủ nhật'), t('Thứ hai'), t('Thứ ba'), t('Thứ tư'), t('Thứ năm'), t('Thứ sáu'), t('Thứ bảy')].map((day, index) => <Checkbox key={day} checked={shiftDays.includes(index)} onChange={event => setShiftDays(current => event.target.checked ? [...current, index] : current.filter(value => value !== index))}>{day}</Checkbox>)}</fieldset>
    ...
    {error && !zoneError && <p role="alert" className="error">{error}</p>}
  </TabbedFormDialog>;
}
