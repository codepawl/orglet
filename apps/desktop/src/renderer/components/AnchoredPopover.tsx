...
/**
 * Orglet popover attached to a trigger: it floats beside the anchor instead of pushing the layout down, flips above when
 * there is no room below, and closes on a click outside or Escape. It sets `data-popup-open`, so Escape inside it closes
 * the popover and not the dialog around it.
 */
export function AnchoredPopover({ anchor, open, onClose, label, className, children }: {
  anchor: RefObject<HTMLElement | null>; open: boolean; onClose: () => void; label: string; className?: string; children: ReactNode;
}) {
  ...
  const classes = ['anchored-popover', placement?.above ? 'above' : '', className ?? ''].filter(Boolean).join(' ');
  return createPortal(
