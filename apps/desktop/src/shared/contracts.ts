...
export type TaskDetail = { task: Task; runs: Run[]; events: Activity[]; artifacts: Artifact[]; profiles: ProfileRecord[]; preflights: PreflightRecord[]; sources: Source[]; workspaceEvidence: (import('./workspace-evidence').WorkspaceReadEvidence & { grantCurrent: boolean })[]; /** App changes the chat's workers proposed, with what became of each (COD-199). */ appProposals: AppProposal[]; usage: Usage; /** What the chat's browser shows right now: a card waiting on the person, the take-over (COD-261); only the `task` command fills it. */ browser?: BrowserLive };
