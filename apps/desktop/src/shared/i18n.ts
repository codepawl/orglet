 ...
 * Exact keys win; otherwise keys with placeholders are matched as patterns (compiled once per dictionary).
 */
export function translateMessage(dictionary: Dictionary | null, message: string) {
  ...
  if (!patterns) {
    patterns = Object.entries(dictionary).filter(([key]) => /\{\d+\}/.test(key)).map(([key, template]) => ({
