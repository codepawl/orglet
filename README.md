...
When workspace commands run, chat shows a short count from their saved exit states: exit code 0, failed, or unfinished. File conflicts and uncertain tool calls remain visible beside that count; Details retains the commands, output and recovery controls. When a command that ran after the last edit fails, the changes wait: the chat shows the answer, names the command and its exit code with its output one click away, and offers **Apply anyway**, **Ask to fix** or **Retry**. A planned check in the turn goal is still only a plan until evidence is recorded.
