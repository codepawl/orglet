...
| 🔁 **Repeat work on a schedule** | Schedules send the same request every day or week while Orglet is open. If the computer was off, missed runs become one catch-up you can run or skip; the next time stays on the calendar. A schedule can instead start when a new file lands in a folder you pick, or only when `orglet run` calls it; those fire only while the app is open and never replay what happened while it was closed ([how](docs/routines.md#what-starts-a-routine)). |
