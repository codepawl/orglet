...
- Local harness detection and runs use `child_process` for installed Claude Code / Codex CLIs only (see `docs/capabilities.md`). `shell.openExternal` is used only for two fixed provider pricing URLs (`orglet:open-pricing`, the renderer passes just `openai` or `anthropic`). No `console` logging in app source. Model traffic goes only through the official OpenAI and Anthropic SDK adapters, created in the core after the main process hands over a decrypted key for that request.
...
- API keys: read from a user-chosen `.txt`, encrypted with Electron `safeStorage` (DPAPI on Windows, Keychain on macOS), never returned to the renderer, excluded from backups and templates.
