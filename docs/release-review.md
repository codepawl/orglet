...
- API keys: typed once in Settings or read from a user-chosen `.txt`, encrypted with Electron `safeStorage` (DPAPI on Windows, Keychain on macOS), never returned to the renderer, excluded from backups and templates. Typed drafts are cleared after a successful save.
