...
- API keys: typed once in Settings or read from a user-chosen `.txt`, encrypted with Electron `safeStorage` (DPAPI) in the main process, never returned to the renderer, excluded from backups and templates. Typed drafts are cleared after a successful save.
