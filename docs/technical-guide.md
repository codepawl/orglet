...
While a run works, the chat shows one island under the worker's byline: a dark pill with a glowing shape and one line for what the worker is doing now (thinking, reading a named file, searching, listing files, running a step, writing the reply, stopping), and above it one grey line for the last step that finished. It says only what the core observed. Claude Code streams its steps (`apps/desktop/src/core/harness/claudeStream.ts`), so reads and searches are named; Codex streams reasoning and the answer only, and an API provider reports nothing finer than planning, combining and waiting, so those runs get an island with fewer states and no receipt line. The full step list, the elapsed time and the worker's notes sit behind one folded control under the answer (the step count, or **Chi tiết** when there are no steps). With `prefers-reduced-motion` every change is a cut.
