...
Today those three choices are pinned in `apps/desktop/src/core/adapters/catalog.ts` (`gpt-4.1-mini-2025-04-14`, `claude-haiku-4-5-20251001`, `grok-3-mini`). Core can already fetch each provider's own model list (`modelList`), cache it for 24 hours in SQLite `settings.modelLists`, and always accept a typed custom ID if the fetch fails. The worker dialog still labels the pinned IDs until [COD-28](https://linear.app/codepawl/issue/COD-28). See [model-list-fetch.md](model-list-fetch.md). A saved key does not establish that the provider account has credits. No subscription credentials are imported.
...
- A worker has at most six model requests per run and 4,096 output tokens per request. It cannot execute code, use a shell, browse websites or submit changes to another service.
...
Public/private ranks use mean completed scores only when both named splits cover the same solutions with completed observations. Ties share competition ranks; public rank minus private rank is the number of places gained. Direction is supplied explicitly, not inferred from the metric name. These are descriptive observations, not a stability threshold or approval. The checker does not rerun solutions, recompute challenge metrics, authenticate logs or establish independent trials.
