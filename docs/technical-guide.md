...
Today those three choices are pinned in `apps/desktop/src/core/adapters/catalog.ts` (`gpt-4.1-mini-2025-04-14`, `claude-haiku-4-5-20251001`, `grok-3-mini`). The plan to fetch each provider's own model list, cache it locally, and always allow a typed custom ID is [model-list-fetch.md](model-list-fetch.md) (COD-29; not implemented yet). A saved key does not establish that the provider account has credits. No subscription credentials are imported.
