...
Each section's header has an edit (pencil) button beside **+**. It turns that section's select mode on: every row shows a checkbox where its status mark was, clicking a row picks it instead of opening its chat, and the pencil becomes a check (Done). Outside select mode, Ctrl-click (Cmd on macOS) picks or unpicks a row and Shift-click picks every row from the last picked one to it; a plain click still opens the chat, and drops the selection. A selection belongs to one section: picking an orglet drops any crews picked.
