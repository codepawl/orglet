...
- **The changed-files line** sits under the answer when the run changed files in its working copy: **Đã thay đổi 3 tệp · +42 −7** (`ChangedFilesLine`). Moves and deletions have their own counts (COD-254): **Đã thay đổi 6 tệp · 5 chuyển hoặc đổi tên · 1 đã xóa**, and a plain folder copy, which has no line counts, leaves the `+ −` part out. A run that only created or removed folders says **Đã thay đổi 2 thư mục**. It opens the diff viewer. Nothing is shown when nothing changed. In a crew turn each member's line is named ("Writer đã thay đổi 2 tệp · +10 −1"), because each member works in its own copy.
