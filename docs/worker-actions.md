...
| command | *Scout đang chạy pnpm test…* | *Lệnh cuối thoát 0* (already in the turn's outcome line) | terminal | outcome line built; live sentence later |
...
- **Changes held for review** ([COD-279](https://linear.app/codepawl/issue/COD-279)): in a solo chat, by default, the run's hand-in stops once the plan is made, the answer is saved, and the changed-files line ends with **Chưa áp dụng vào thư mục · Xem lại** ("Not in your folder yet · Review"), the last word in the text colour and heavier so it reads as the thing to click. The line opens the viewer with **Bỏ thay đổi** and **Áp dụng** in its toolbar. Once decided, the line ends with what happened: **Đã áp dụng**, **Đã áp dụng, bỏ qua 1**, or **Đã bỏ, thư mục không đổi**. A turn whose copy a later turn went on in keeps its counts as plain text ending **Chuyển sang lượt sau** ("Carried into the next turn") and opens nothing, since the later turn's diff shows both. The rules are in [agent-tools.md](agent-tools.md#reviewing-changes-before-they-reach-the-folder).
- **Commands** are summed in the turn's outcome line, the last command first and then the earlier ones by kind ("Lệnh cuối thoát 0 · trước đó: 1 lần lỗi", `commandTally` in `shared/work-outcomes.ts`), with their output under **Details**.
...
- **What is kept**: the saved events, the answer or report, the working copy's counts (`WorkspaceRecoveryView.copies[].diff`), what became of held changes (`copies[].review`, `copies[].carried`), and the copy itself on disk until the chat is deleted, so the diff can be opened later, discarded changes included. Thinking and the live progress are not kept.
...
- **Collapsed**, one muted line counts each kind of thing that happened, in a fixed order, each count with its own words for one and for several: *Dùng 1 ghi nhớ · Nạp 2 ghi chú · Đọc 2 tệp · Tìm 1 lần · Tìm trên web 1 lần · Đọc 3 trang web · Ghi nhớ thêm 1 điều* ("Used 1 memory · Loaded 2 notes · Read 2 files · Searched once · Searched the web once · Read 3 web pages · Remembered 1 thing"). Web searches and web pages count apart; one count for both, "Went online 6 times", did not say what the orglet did. With nothing to count there is no control at all.
...
- **Viewer.** `DiffViewer` (`renderer/components/DiffViewer.tsx`) opens like `SourceViewer`: close on the left, "Thay đổi của Scout" and "3 tệp · +42 −7" in the middle, the info button on the right. Redrawn in COD-279 after the owner called it ugly, as a code review surface in the app's quiet style: on the grey stage, one card lists the changed files (left out when one file changed, since its header already names it), each row the file's kind icon, its path with the folder muted and the name in full weight (the folder shortens first), a small tinted word for what happened (new in the success colour, deleted in the error colour, moved or renamed plain) and its counts; a click jumps to the file. Then one card per file, whose header (fold chevron, icon, path, status, counts) stays at the top of the viewer while its lines scroll under it and folds the file away. Each hunk opens with its range and Git's function heading on a faint accent band aligned with the code, then one grid row per line: the snapshot's and the copy's line numbers, the sign, and the code, which wraps inside its column so a long line never widens the viewer; changed lines take a light tint of the error or success colour and their numbers a slightly stronger one, with the same tokenizer as every other code view (`highlight.ts`). Every count in the viewer, the toolbar's included, wears the diff's colours like the chat's line (`DiffCounts`). Binary files, a cut diff and a file with no line changes each get one quiet note inside their card. Reading it never touches the folder. For changes held for review it also carries the decision (COD-279): a line under the toolbar says nothing has reached the folder yet, **Bỏ thay đổi** (outline, asks once with `confirmAction`) and **Áp dụng** (primary) sit left of the info button, and with more than one file every file gets a kit `Checkbox`, all ticked, in the list and in its header (one state). Folders have no tick; they follow their files. Unticking some makes the button **Áp dụng 2/3** ("Apply 2 of 3"); a moved file's row sends both of its paths. With a single file there is no list and no tick. Keeping current files and conflicts stay in **Details** (`WorkspaceRecovery`), and **Vẫn áp dụng** on a turn whose hand-in a failed command blocked still sits with the turn's actions.
