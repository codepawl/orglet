...
beforeEach(async () => {
  ...
  failReviewer = false; calls = []; live = 0; peak = 0;
...
async function done(taskId: string) {
  for (let i = 0; i < 100 && core.teams.isActive(taskId); i++) await new Promise(resolve => setTimeout(resolve, 10));
...
it('joins independent artifacts, keeps source scopes local and uses at most two parallel members', async () => {
  ...
  expect(detail.task.status).toBe('completed'); expect(detail.runs).toHaveLength(3); expect(detail.artifacts).toHaveLength(3);
  expect(peak).toBe(2);
...
it('preserves a failed role as partial and retries only missing members before a new join', async () => {
  ...
  const after = store.detail(taskId); expect(after.task.status).toBe('completed'); expect(after.runs).toHaveLength(5);
...
it('sequential members receive only already committed results from their task', async () => {
  ...
  expect(peak).toBe(1); expect(detail.runs[0].snapshot.upstreamArtifactIds).toEqual([]); expect(detail.runs[1].snapshot.upstreamArtifactIds).toHaveLength(1);
  expect(detail.runs[2].snapshot.upstreamArtifactIds).toHaveLength(2);
});
...
it('starts fresh roles for an input revision while retaining previous artifacts and total usage', async () => {
  ...
  expect(after.task.inputRevision).toBe(1); expect(after.runs).toHaveLength(6); expect(after.artifacts).toHaveLength(6);
  expect(after.artifacts.slice(0, 3)).toEqual(oldArtifacts);
  expect(after.runs.slice(3).every(run => run.snapshot.input?.brief === 'Reassess with a new instruction' && run.snapshot.inputRevision === 1)).toBe(true);
